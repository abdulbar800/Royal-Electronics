...
const Checkout = () => {
  ...
  const cancelPendingOrder = async (pendingOrderId) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/orders/${pendingOrderId}/cancel-pending`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`
      ...
      if (res.ok) {
        ...
        console.error('Cancel-pending FAILED — status:', res.status, 'response:', data);
      }
    } catch (err) {
      console.error('Cancel pending order network error:', err);
    }
  };

  // =====================================================
  // RAZORPAY PAYMENT - Backend se Key uthayega
  // =====================================================
        ...
        handler: async function(response) {
          try {
            const confirmRes = await fetch(
              `http://localhost:5000/api/orders/${currentOrderId}/confirm-payment`,
              {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!orderRes.ok) {
        alert('❌ ' + (orderDataRes.message || 'Order failed'));
        setSubmitting(false);
        return;
      }
